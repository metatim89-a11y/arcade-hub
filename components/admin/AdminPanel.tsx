...
interface AdminDashboard {
  ...
  players: Array<{ id: string; displayName: string | null; email?: string | null; status: string; createdAt: string; lastSeenAt: string | null; funCoins: number }>;
}
...
const AdminPanel: React.FC<AdminPanelProps> = ({ onClose }) => {
  ...
  const loadDashboard = async () => {
    try {
      ...
      if (rpcError) {
        console.warn('Direct get_admin_dashboard RPC notice:', rpcError.message);
        // Fallback: Query profiles table directly
        const { data: profiles } = await getSupabase().from('profiles').select('id, display_name, status, created_at, last_seen_at');
        if (profiles && profiles.length > 0) {
          setDashboard({
            totals: { players: profiles.length, events: 0, uniqueVisitors: profiles.length },
            eventsByType: {},
            players: profiles.map((p: any) => ({
              id: p.id,
          ...
          setDashboard({
            totals: { players: 1, events: 0, uniqueVisitors: 1 },
            eventsByType: {},
            players: [{
              id: user?.id || 'admin_tim',
              displayName: user?.username || 'Tim',
              email: user?.email || 'tim@arcadehub.local',
              status: 'active',
          ...
          <section className="admin-players-card">
            ...
            <div className="admin-player-list">
              {(dashboard?.players ?? []).map((player) => (
                <button type="button" key={player.id} onClick={() => openPlayer(player)} disabled={player.id === user?.id}>
                  <div>
                    <strong>{player.displayName || 'Unnamed player'}</strong>
                    {player.email && <div style={{ color: '#71c5ee', fontSize: '9px', fontFamily: 'monospace', marginTop: '2px' }}>{player.email}</div>}
                  </div>
                  <span>{player.status} · {Math.floor(player.funCoins)} FC</span>
                  <small>{player.id === user?.id ? 'Your admin account' : new Date(player.createdAt).toLocaleDateString()}</small>
                </button>
              ))}
