...
export const PlinkoBoard3D: React.FC<{
  ...
  useEffect(() => {
    ...
    void import('three').then((THREE) => {
      ...
      [[-5.25, 0, .25, 8.8], [5.25, 0, .25, 8.8], [0, 4.35, 10.5, .24], [0, -4.35, 10.5, .24]].forEach(([x, y, width, height]) => { const rail = new THREE.Mesh(new THREE.BoxGeometry(width, height, .58), railMaterial); rail.position.set(x, y, .04); rail.castShadow = true; boardRoot.add(rail); });
      ...
      const marquee = new THREE.Mesh(new THREE.BoxGeometry(3.2, .62, .28), new THREE.MeshStandardMaterial({ map: marqueeTexture, emissive: 0xd39118, emissiveIntensity: .25, metalness: .25, roughness: .3 })); marquee.position.set(0, 4.68, .12); marquee.castShadow = true; boardRoot.add(marquee);
      const cabinetLights: Mesh[] = [];
      for (let index = 0; index < 18; index += 1) { const bulb = new THREE.Mesh(new THREE.SphereGeometry(.085, 12, 9), new THREE.MeshStandardMaterial({ color: 0x69d9ff, emissive: 0x2cbfff, emissiveIntensity: .8 })); bulb.position.set(index < 9 ? -5.13 : 5.13, 3.7 - (index % 9) * .92, .42); bulb.castShadow = true; boardRoot.add(bulb); cabinetLights.push(bulb); }
      ...
      const animate = (now: number) => {
        ...
        const averageX = ballsRef.current.length ? ballsRef.current.reduce((sum, ball) => sum + (ball.x / width - .5) * 10, 0) / ballsRef.current.length : 0; cameraX += (averageX * .025 - cameraX) * (1 - Math.exp(-3 * delta)); camera.position.x = cameraX; camera.lookAt(cameraX * .25, -.15, 0);
