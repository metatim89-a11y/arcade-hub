...
const NimGame: React.FC<NimGameProps> = ({ playMode, playerNames }) => {
  ...
  return <div className="flex w-full max-w-xl flex-col items-center gap-5 px-3 text-white">
    ...
    <div className="grid w-full gap-3 rounded-2xl border border-cyan-300/20 bg-slate-950/60 p-4">
      {piles.map((pile, row) => <div key={row} className="flex min-h-16 items-center gap-3 rounded-xl bg-cyan-950/50 p-3">
        <span className="w-16 text-xs font-black uppercase text-cyan-200">Pile {row + 1}</span>
        <div className="flex flex-1 flex-wrap gap-2">{Array.from({ length: pile }, (_, index) => <button key={index} type="button" disabled={!canAct} onClick={() => setSelected({ row, count: pile - index })} className={`h-9 w-9 rounded-full border-2 transition ${selected?.row === row && index >= pile - (selected.count) ? 'border-yellow-200 bg-yellow-300 text-slate-950' : 'border-cyan-200/50 bg-cyan-600 text-white'} disabled:opacity-60`} aria-label={`Take ${pile - index} from pile ${row + 1}`}>●</button>)}</div>
        <button type="button" disabled={!canAct || selected?.row !== row} onClick={() => selected && take(row, selected.count)} className="rounded-lg bg-yellow-300 px-3 py-2 text-xs font-black text-slate-950 disabled:opacity-40">TAKE</button>
      </div>)}
    </div>
