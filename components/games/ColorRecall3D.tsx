  ...
  useEffect(() => {
    ...
    void import('three').then((THREE) => {
      ...
      const animate = (now: number) => {
        ...
        pads.forEach((pad, index) => { const active = live.activePad === index; const hover = hovered === index && live.phase === 'INPUT'; const material = pad.material as InstanceType<typeof THREE.MeshPhysicalMaterial>; const targetY = active ? .58 : hover ? .34 : Number(pad.userData.homeY); pad.position.y += (targetY - pad.position.y) * .22; pad.rotation.x += ((active ? -.045 : hover ? -.018 : 0) - pad.rotation.x) * .18; pad.scale.setScalar(active ? 1.045 : hover ? 1.018 : 1); material.emissiveIntensity = active ? 1.5 + Math.sin(now * .035) * .25 : hover ? .38 : .14; });
        ...
        root.rotation.y += (pointerTarget.x * .035 - root.rotation.y) * .06; root.rotation.x += (-pointerTarget.y * .018 - root.rotation.x) * .06; renderer.render(scene, camera); frame = requestAnimationFrame(animate);
      };
