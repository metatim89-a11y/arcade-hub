...
const PlinkoGame: React.FC = () => {
    ...
    return (
        <div className={`plinko-game theme-${theme.toLowerCase()} flex flex-col lg:flex-row gap-4 lg:gap-6 w-full max-w-7xl items-start justify-center p-2 sm:p-4`}>
            <div className="plinko-hub w-full lg:w-[340px] flex flex-col gap-3 sm:gap-4 p-4 sm:p-6 rounded-2xl h-fit">
                ...
                <div className="plinko-setting bg-black/30 p-4 rounded-xl border border-gray-700">
            ...
            <div className="flex-1 flex flex-col gap-3 sm:gap-4 w-full max-w-[800px]">
                ...
                <div className={`plinko-board relative w-full bg-gray-900 rounded-2xl border-4 border-gray-800 shadow-2xl overflow-hidden flex justify-center min-h-[360px] ${followBall ? 'following' : ''} ${challengeBoard ? 'challenge' : ''}`}>
            ...
            <style>{`.plinko-hub{border:1px solid color-mix(in srgb,var(--stage-accent,#fbbf24) 44%,#374151);background:radial-gradient(circle at 50% -10%,color-mix(in srgb,var(--stage-accent,#fbbf24) 18%,transparent),transparent 40%),linear-gradient(155deg,rgba(17,24,39,.94),rgba(4,8,15,.9));box-shadow:inset 0 1px rgba(255,255,255,.08),0 24px 55px rgba(0,0,0,.4),0 0 24px color-mix(in srgb,var(--stage-accent,#fbbf24) 10%,transparent);backdrop-filter:blur(15px)}.plinko-options,.plinko-batch,.plinko-toggles{display:grid;grid-template-columns:repeat(2,1fr);gap:5px}.plinko-options select,.plinko-batch button,.plinko-toggles button,.plinko-replay{padding:7px;border:1px solid #4b5563;border-radius:7px;background:linear-gradient(#243244,#151e2a);color:#cbd6e1;font-size:10px;font-weight:850}.plinko-batch{grid-template-columns:auto repeat(3,1fr);align-items:center}.plinko-batch span{color:#8994a3;font-size:10px}.plinko-batch button.active,.plinko-toggles button.active{border-color:#fbbf24;color:#ffe181;box-shadow:inset 0 0 12px rgba(251,191,36,.13),0 0 10px rgba(251,191,36,.12)}.plinko-daily,.plinko-preview{padding:8px;border:1px solid #293747;border-radius:7px;background:#0a121e;color:#9caebf;text-align:center;font-size:10px}.plinko-replay{white-space:nowrap}.plinko-board{border-color:#263b50;background:radial-gradient(circle at 50% 10%,#142c42,#050b13 70%);box-shadow:inset 0 0 55px rgba(0,0,0,.62),0 26px 55px rgba(0,0,0,.44),0 0 0 1px rgba(99,208,255,.16)}.plinko-board.challenge:after{content:'CHALLENGE BOARD';position:absolute;right:8px;bottom:8px;padding:4px 6px;border-radius:5px;background:#7c3aed;color:white;font-size:8px;font-weight:900}.plinko-board.following canvas{filter:saturate(1.25) brightness(1.1)}.plinko-heatmap{display:grid;grid-template-columns:repeat(${multipliers.length},1fr);gap:2px;height:10px}.plinko-heatmap i{border-radius:3px;background:linear-gradient(#ffe36a,#e58b19);box-shadow:0 0 8px rgba(245,158,11,.32)}.theme-neon .plinko-board{box-shadow:inset 0 0 50px rgba(0,0,0,.5),0 0 42px rgba(34,211,238,.34)}.theme-candy .plinko-board{border-color:#ec4899;background:radial-gradient(circle at top,#51204e,#210c2d 72%)}@media(max-width:640px){.plinko-game{gap:10px!important}.plinko-board{min-height:320px}.plinko-hub{padding:10px!important;gap:8px!important}.plinko-hub h2{margin-bottom:0!important;font-size:20px}.plinko-setting{padding:8px!important}.plinko-hub [class~='py-4']{padding-top:10px!important;padding-bottom:10px!important}.plinko-daily{padding:5px}}`}</style>
        </div>
    );
};
