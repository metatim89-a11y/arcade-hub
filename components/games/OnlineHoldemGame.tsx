...
import { getBetFlights, getHoldemAnimationCue, type HoldemAnimationCue } from '../../lib/animationCues';
...
const OnlineHoldemGame: React.FC<{ tableId: string; userId: string; onLeave: () => void }> = ({ tableId, userId, onLeave }) => {
  ...
  const [animationCue, setAnimationCue] = useState<HoldemAnimationCue>(null);
  const [betFlights, setBetFlights] = useState<Array<{ id: number; playerId: string; amount: number }>>([]);
  const previousRef = useRef<{ hand: number; boardCount: number; actor: number; bets: Array<{ id: string; bet: number }> } | null>(null);
  ...
  useEffect(() => {
    ...
    const next = { hand: state.hand, boardCount: state.board.length, actor: state.actor };
    const cue = getHoldemAnimationCue(previous, next);
    const flights = previous ? getBetFlights(previous.bets, state.players.map(({ id, bet }) => ({ id, bet }))) : [];
    previousRef.current = { ...next, bets: state.players.map(({ id, bet }) => ({ id, bet })) };
    ...
    if (cue) {
      ...
      }, cue === 'deal' ? 1150 : cue === 'street' ? 760 : 360);
    }
    if (flights.length) {
      if (flightTimerRef.current) window.clearTimeout(flightTimerRef.current);
      setBetFlights(flights.map((flight) => ({ ...flight, id: ++flightIdRef.current })));
  ...
  const animationStatus = animationCue === 'deal'
    ? 'DEALING HOLE CARDS…'
    : animationCue === 'street'
      ? `DEALING ${state.phase}…`
      : null;
  const animating = animationCue !== null;
  const chipVectors = [{ x: '0px', y: '-120px' }, { x: '120px', y: '0px' }, { x: '0px', y: '120px' }, { x: '-120px', y: '0px' }];
    ...
    <section className="online-table-game canonical-holdem-hud">
      ...
      <div className="online-room">
        <div className="online-felt">
          <div className="online-table-3d"><PokerTable3D players={tablePlayers} community={tableBoard} showdown={state.phase === 'SHOWDOWN'} activeIndex={state.actor} dealerIndex={state.dealer} handId={state.hand} /></div>
          ...
          {state.players.map((player, index) => (
            <div key={player.id} className={`online-seat seat-${index}${index === state.actor ? ' acting' : ''}${player.folded ? ' folded' : ''}`}>
              {index === state.actor && <i key={`turn-${state.hand}-${index}`} className="turn-marker">{player.id === userId ? 'YOUR TURN' : `${player.name}’S TURN`}</i>}
              ...
              <div className="canonical-badges">
                ...
                {betFlights.filter((flight) => flight.playerId === player.id).map((flight) => (
                  <i
                    key={flight.id}
                    className="bet-flight"
                    style={{ '--chip-x': chipVectors[index]?.x ?? '0px', '--chip-y': chipVectors[index]?.y ?? '-120px' } as React.CSSProperties}
                  >
                    +{flight.amount}
                  </i>
                ))}
      ...
      <footer className="canonical-controls">
        {animationStatus && state.phase !== 'SHOWDOWN' ? <span className="dealing-status">{animationStatus}</span> : state.phase === 'SHOWDOWN' ? <>
