...
import SpinWheel3D from './SpinWheel3D';
...
const SpinWheelGame: React.FC = () => {
    ...
    return (
        <section className="wheel-game">
            ...
            <div className="wheel-glass"><canvas ref={canvasRef} className="wheel-physics" aria-hidden="true" /><SpinWheel3D rotationRef={rotationRef} segments={SEGMENTS} winningIndex={winningIndex} spinning={isSpinning} onSpin={() => void handleSpin()} /></div>
            ...
            <style>{`
              .wheel-game{width:min(100%,500px);margin:auto;padding:16px;border:1px solid rgba(210,165,70,.42);border-radius:22px;background:linear-gradient(145deg,rgba(35,29,39,.72),rgba(10,13,18,.76));backdrop-filter:blur(18px) saturate(1.25);box-shadow:inset 0 1px rgba(255,255,255,.15),0 25px 65px rgba(0,0,0,.42);text-align:center}.wheel-kicker{color:#cfa94e;font-size:9px;font-weight:900;letter-spacing:.2em}.wheel-game h2{margin:3px 0 10px;color:#f4e6b7;font-size:25px}.wheel-glass{position:relative;width:min(64vw,290px);margin:auto;padding:9px;border:1px solid rgba(255,255,255,.2);border-radius:50%;background:radial-gradient(circle at 32% 20%,rgba(255,255,255,.2),rgba(255,255,255,.04) 48%,rgba(77,48,12,.18));box-shadow:inset 0 0 28px rgba(255,255,255,.1),0 18px 40px rgba(0,0,0,.4);backdrop-filter:blur(12px)}.wheel-glass:after{content:'';position:absolute;inset:9% 20% 55%;border-radius:50%;background:linear-gradient(rgba(255,255,255,.25),transparent);pointer-events:none;transform:rotate(-12deg)}.wheel-glass canvas{display:block;width:100%;aspect-ratio:1}.wheel-glass .wheel-physics{visibility:hidden}.wheel-feedback{display:flex;align-items:center;justify-content:center;min-height:40px;margin:10px auto 9px;padding:8px 11px;border:1px solid rgba(211,169,75,.3);border-radius:9px;background:rgba(7,9,12,.42);color:#e7d49c;font-size:12px}.wheel-controls{display:grid;grid-template-columns:1fr 38px 90px 38px;align-items:center;gap:6px;max-width:360px;margin:auto}.wheel-controls label{color:#a99565;font-size:9px;font-weight:900;letter-spacing:.12em;text-align:left}.wheel-controls label span{color:#e5c765}.wheel-controls button,.wheel-controls input{height:38px;border:1px solid rgba(204,160,63,.4);border-radius:7px;background:rgba(11,14,17,.55);color:#f2dfaa;text-align:center;font-weight:900}.wheel-controls button{cursor:pointer}.wheel-spin{width:min(100%,360px);margin-top:9px;padding:12px;border:1px solid #d6a73b;border-radius:9px;background:linear-gradient(#e6c15d,#a97419);box-shadow:0 5px 0 #5c3d0a;color:#281a05;font-weight:950;letter-spacing:.08em;cursor:pointer}.wheel-spin:disabled,.wheel-controls button:disabled{opacity:.48;cursor:not-allowed}@media(max-width:480px){.wheel-game{padding:10px}.wheel-glass{width:min(68vw,240px);padding:6px}.wheel-controls{grid-template-columns:1fr 34px 74px 34px}}
            `}</style>
        </section>
    );
};
